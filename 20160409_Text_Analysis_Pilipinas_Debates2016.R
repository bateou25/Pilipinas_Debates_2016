# The recent 1st Philippines' Presidential Debate (Pilipinas Debates 2016) drew unprecedented amounts of viewers and the
...
#It would actually be interesting to see for each candidate, the words in his top-50
#that are unique to him, i.e. that are not in the other candidates' top-50
...
#WORDCLOUD
freq = dtm['BINAY', l$BINAY]
wordcloud(l$BINAY, as.vector(freq), colors=brewer.pal(9,'Blues'))

freq = dtm['DUTERTE', l$DUTERTE]
wordcloud(l$DUTERTE, as.vector(freq), colors=brewer.pal(9,'Greens'))

freq = dtm['POE', l$POE]
wordcloud(l$POE, as.vector(freq), colors=brewer.pal(9,'Oranges'))
...
# BINAY  MIRIAM DUTERTE     POE     MAR 
# 488.2   612.8   537.7   466.3   478.6 

#We can see that Poe and Mar talked the most during the first debate, with
#Duterte and Binay  following them behind. But Poe and Mar used the
#fewest words (i.e. unique words) than the rest, with Binay at 3rd (recycled words??)

#We can also see that almost all of the candidates used short words.
