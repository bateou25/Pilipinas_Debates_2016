...
ref_matrix = function (date){
  ...
  # prez = tm_map(prez, removeWords, c('Laughter', 'Applause', 'Intermission'))
...
unique_words_freq = word_count >= 1
...
wordcloud(l_freq$BINAY, as.vector(freq), colors=brewer.pal(9,'Blues'))

freq = dtm['DUTERTE', l_freq$DUTERTE]
wordcloud(l_freq$DUTERTE, as.vector(freq), colors=brewer.pal(9,'Greens'))

freq = dtm['POE', l_freq$POE]
wordcloud(l_freq$POE, as.vector(freq), colors=brewer.pal(9,'Oranges'))

freq = dtm['MAR', l_freq$MAR]
wordcloud(l_freq$MAR, as.vector(freq), colors=brewer.pal(9,'Greens'))

freq = dtm['MIRIAM', l_freq$MIRIAM]
wordcloud(l_freq$MIRIAM, as.vector(freq), colors=brewer.pal(9,'Blues'))
...
#We see that Poe and Mar were the most active for the first and second debates (59% of the total words thrown; with dups)
#They also used longer words (7.1 and 6.9 average word length, respectively).With Binay included, these three candidates
#used longer words than the rest of their opponents.

#In terms of the diversity of words used (per 1000 words), this same set of candidates + Duterte used fewer words
#except Miriam. We see that Miriam was the least active, but she have the most diversified vocabulary among
#the five candidates. It came as a surprise, knowing that she was not around during the 2nd run of the debates.
